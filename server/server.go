...
func (s *Server) setupRouter() {
	...
	r.HandleFunc("/voteComment", responder(s.postCommentVote))
	r.HandleFunc("/viewUsersReport", responder(s.getUsersReport))
	r.HandleFunc("/viewFeedback", responder(s.getFeedback))
	r.HandleFunc("/toggleFeedbackHidden", responder(s.toggleFeedbackHidden))
	r.HandleFunc("/newFeedback", responder(s.postFeedback))
	r.HandleFunc("/assignGlobalModerator", responder(s.postAssignGlobalModerator))
	r.HandleFunc("/assignAdmin", responder(s.assignAdmin))
	r.HandleFunc("/viewCommentReports", responder(s.getCommentReports))
	r.HandleFunc("/newReport", responder(s.postCommentReport))
	...
	r.HandleFunc("/viewUser", responder(s.postViewUser))
	r.HandleFunc("/amILoggedIn", responder(s.postAmILoggedIn))
	r.HandleFunc("/requestVerificationCode", responder(s.postRequestVerificationCode))
	r.HandleFunc("/inputVerificationCode", responder(s.postInputVerificationCode))
