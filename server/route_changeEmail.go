...
func (c *MemberControllerBase) HandleCommandChangeEmail(comm *communication.ChangeEmail, server *Server) {
	if comm.Password != c.User.Password {
		...
		err := server.DB.Queries.UpdateUserEmail(context.Background(), params)
		if err == nil {
			c.nextResponse = append(c.nextResponse, communication.GetMessage(false, "Failed to change email."))
		} else {
			c.nextResponse = append(c.nextResponse, communication.GetMessage(true, "Email updated."))
