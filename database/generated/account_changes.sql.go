...
func (q *Queries) CreateVerificationCode(ctx context.Context, arg CreateVerificationCodeParams) (VerificationCode, error) {
	row := q.db.QueryRowContext(ctx, createVerificationCode, arg.UserID, arg.ID)
...
const deletePreviousVerificationCodesForUser = `-- name: DeletePreviousVerificationCodesForUser :exec
DELETE FROM "VerificationCodes"
WHERE user_id = $1
`

func (q *Queries) DeletePreviousVerificationCodesForUser(ctx context.Context, userID int64) error {
	_, err := q.db.ExecContext(ctx, deletePreviousVerificationCodesForUser, userID)
	return err
}
...
const getVerificationCodeEntry = `-- name: GetVerificationCodeEntry :one
SELECT id, user_id, created_on FROM "VerificationCodes"
WHERE "id" = $1 LIMIT 1
`

func (q *Queries) GetVerificationCodeEntry(ctx context.Context, id int64) (VerificationCode, error) {
	row := q.db.QueryRowContext(ctx, getVerificationCodeEntry, id)
	var i VerificationCode
	err := row.Scan(&i.ID, &i.UserID, &i.CreatedOn)
	return i, err
}
...
func (q *Queries) UpdateVerification(ctx context.Context, arg UpdateVerificationParams) error {
	_, err := q.db.ExecContext(ctx, updateVerification, arg.ID, arg.IsVerified)
	return err
}
